...
namespace ImpulsaDBA.Services
{
    public class AsignaturaService
    {
        ...
        public AsignaturaService(DatabaseService databaseService)
        {
        ...
        /// </summary>
        public async Task<List<Asignatura>> ObtenerAsignaturasPorProfesor(int profesorId)
        {
            try
            {
                var query = @"
                ...
                foreach (DataRow row in result.Rows)
                {
                    ...
                    {
                        Id = Convert.ToInt32(row["id_asignatura"]),
                    ...
                    asignatura.Estadisticas = await ObtenerEstadisticasActividades(asignatura.IdAsignacionAcademica);
        ...
        public async Task<EstadisticasActividades> ObtenerEstadisticasActividades(int idAsignacionAcademica)
        {
            try
            {
                ...
                // Obtener estadísticas de actividades del año y periodo
                var query = @"
                    SELECT 
                        COUNT(*) AS totales,
                        SUM(CASE WHEN A.eliminado = 0 AND CAST(A.fec_inicio AS DATE) >= CAST(GETDATE() AS DATE) THEN 1 ELSE 0 END) AS activas,
                        SUM(CASE WHEN A.eliminado = 1 THEN 1 ELSE 0 END) AS inactivas,
                        SUM(CASE WHEN A.eliminado = 0 AND CAST(A.fec_inicio AS DATE) < CAST(GETDATE() AS DATE) THEN 1 ELSE 0 END) AS pendientes
